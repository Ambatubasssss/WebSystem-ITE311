...
class AddCourseRelationshipsToCourses extends Migration
{
    public function up()
    {
        // Add academic relationships to courses table (3NF compliant - using foreign keys)
        $fields = [
            'academic_year_id' => [
                'type'       => 'INT',
                'constraint' => 11,
        ...
        if (!$this->db->fieldExists('academic_year_id', 'courses')) {
            $this->forge->addColumn('courses', $fields);
            
            // Add foreign key constraints (3NF - referential integrity)
            ...
            if ($this->db->tableExists('year_levels')) {
    ...
    public function down()
    {
        ...
        if ($this->db->tableExists('courses')) {
            $this->db->query('ALTER TABLE `courses` DROP FOREIGN KEY IF EXISTS `fk_courses_academic_year`');
            $this->db->query('ALTER TABLE `courses` DROP FOREIGN KEY IF EXISTS `fk_courses_semester`');
            $this->db->query('ALTER TABLE `courses` DROP FOREIGN KEY IF EXISTS `fk_courses_year_level`');
        }
